...
- Requirement: Current Zama SDK + open-source license.
  Acceptance: `@zama-fhe/sdk` tracks the current published line (`^3.2.0` in Phase 005); unused `@zama-fhe/react-sdk` is removed; the repo carries an explicit OSI `LICENSE` (MIT).
...
- Question: Authoritative live relayer host and gateway chain id?
  Status: Partially resolved. Installed `@zama-fhe/sdk` 3.2.0 preset resolves `relayer.testnet.zama.org/v2` / gateway `10901`; `chains.ts` display-only drift is annotated. Confirm actual live traffic via the Network tab during Phase 006 UAT before changing displayed constants or adding strict CSP.

- Question: Which dedicated Sepolia RPC provider and key-exposure model?
  Status: Decided for Phase 005. Use `NEXT_PUBLIC_SEPOLIA_RPC_URL` with a referrer/domain-restricted Alchemy/Infura/dRPC key; no server proxy in this phase. Rebuild/redeploy after changing it.

- Question: License choice?
  Status: Decided. MIT license added with matching `package.json` SPDX metadata.
